        ...
        with st.form("login_form"):
            ...
            if st.form_submit_button("INITIALIZE_SEQUENCE"):
                if u == "admin" and p == "admin123":
                    ...
                    st.rerun()
                elif u == "kelly" and p == "password":
                    st.session_state.logged_in = True
                    st.session_state.username = "kelly"
                    st.rerun()
                else:
                    st.error("INVALID_CREDENTIALS // ACCESS_DENIED")
...
with st.sidebar:
    ...
    tag_filter = st.selectbox("TAG_FILTER", ["ALL"] + allowed_tags)
...
if st.session_state.view == "DASHBOARD":
    ...
    if wf:
        ...
        st.error("NODE_NOT_FOUND")

elif st.session_state.view == "SETTINGS":
    st.markdown("<h1 class='glow'>> SYSTEM_CONFIGURATION</h1>", unsafe_allow_html=True)
    st.checkbox("ENABLE_NEURAL_LINK", value=True)
    st.checkbox("ENCRYPT_DATA_STREAMS", value=True)
    st.slider("SIGNAL_STRENGTH", 0, 100, 85)
    st.selectbox("TERMINAL_THEME", ["MATRIX_GREEN", "ZION_BLUE", "REBEL_RED"])

# --- FOOTER ---
st.sidebar.markdown("---")
st.sidebar.caption("MATRIX_OS // VER_3.5.0")
